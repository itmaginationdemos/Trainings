...
export class CanAccessFileGuard implements CanActivate {
    ...
    async canActivate(
        ...
        const fieldPosition = this.reflector.get<FieldRequestPosition>('fileIdPosition', context.getHandler());

        const fileIdString = request[fieldPosition.position][fieldPosition.field];
        const fileId = parseInt(typeof fileIdString === 'string' ? fileIdString : '');


        const passPosition = this.reflector.get<FieldRequestPosition>('filePasswordPosition', context.getHandler());
