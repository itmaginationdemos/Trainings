...
export class FileManagerController {
    protected readonly uploadDir = join(__dirname, '..', '..', '..', 'public', 'uploads');
    ...
    @Put('/')
    @ApiOkResponse({description: 'Dane utworzonego obiektu', type: () => FileDto})
    @ApiOperation({summary: 'send file', description: 'receives the file and saves on hard drive and database', operationId: 'fileManager_store'})
    ...
    async handleFile(@UploadedFile() file: UploadedFileType) {
        ...
        // dodaj do bazy
        try {
            return this.dbService.create({originalname: file.originalname, mimetype: file.mimetype, size: file.size, ext: file.originalname.substring(file.originalname.length - 4, file.originalname.length) || '', ourname: randomedName,});
        } catch (err) {
            try { // nie chcemy by user widział że nie umieliśmy usunać jego pliku, ktory własnie wysłał do nas
    ...
    @ApiOkResponse({})
    @ApiOperation({description: 'Dokładna instrukcja', summary: 'Szybka informacja'})
    async changePassword(@Param('id', new ParseIntPipe()) id: number, @Body(new ValidationPipe()) dto: ChangePasswordDto) {
    ...
    async getMeta(@Param('fileId', new ParseIntPipe()) id: number) {
        ...
        return Object.assign({}, entity.get({plain: true}), {pass: '--- redacted ---'}); // simple
    ...
    @Get('/:id')
    @ApiParam({name: 'id'})
    async fetchFile(@Param('id', new ParseIntPipe()) id: number, @Res() res: Response) {
        ...
        try {
            ...
            return res.status(500).json({statusCode: 500, message: 'Could not read file'});
        }
    }
}
