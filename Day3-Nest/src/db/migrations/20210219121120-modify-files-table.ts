...
export default {
  up: async (queryInterface: QueryInterface, Sequelize: any) => {
    ...
    if (tablesInDb.indexOf(tableName) !== -1) { // gdy tabela już istnieje
      const tableData = await queryInterface.describeTable(tableName);
      const tableColumns = Object.keys(tableData);

      if (tableColumns.indexOf('path') !== -1) { // istnieje kolumna
      ...
      if (tableColumns.indexOf('extension') !== -1) { // istnieje kolumna
      ...
      if (tableColumns.indexOf('mimetype') !== -1) { // istnieje kolumna
      ...
      if (tableColumns.indexOf('pass') !== -1) { // istnieje kolumna
