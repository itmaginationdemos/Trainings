...
const fillCache = async () => {
    ...
    for (let i = 0; i < filesToCache.length; ++i) {
        try {
            const content = await readFile(path.join(__dirname, 'uploads', filesToCache[i]));
            ...
            cache[meta.name].push({file: filesToCache[i].substring(0, filesToCache[i].length - 4), meta});
...
function start () {
    ...
    app.post('/file/:name', async (req, res) => {
        ...
        try {
            ...
            await writeFile(path.join(__dirname, 'uploads', `${file.file}.txt`), JSON.stringify(file.meta));
