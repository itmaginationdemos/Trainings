...
app.put('/file', fileUploader.single('file'), async (req, res) => {
    ...
    pool.query(
        ...
        [req.file.originalname, req.file.filename, req.file.size, req.file.originalname.substring(req.file.originalname.length - 4)],
        (err, results) => {
...
app.get('/file', async (req, res) => {
    ...
    pool.query('SELECT id, name, size, pass FROM files', (err, response) => {
        ...
        res.json(response.map(r => ({id: r.id, name: r.name, size: r.size, protected: !!r.pass})));
    });
});

app.delete('/file/:id', async (req, res) => {
    pool.query('SELECT path FROM files WHERE id = ?', [req.params.id], function (err, results) {
        if (err) return res.status(500).send(err.message);
        ...
        pool.query('DELETE FROM files WHERE id = ?', [req.params.id], async (err, results) => {
            if (err) return res.status(500).send(err.message);
...
app.post('/file/:id', async (req, res) => {
    pool.query('SELECT pass FROM files WHERE id = ?', [req.params.id], (err, results) => {
        if (err) return res.status(500).send(err.message);
        if (results.length === 0) return res.status(404).json('Not found');
        ...
        pool.query('UPDATE files SET `pass` = ? WHERE id = ?', [req.query.newPass, req.params.id], function (err, results) {
...
app.get('/file/:id', async (req, res) => {
    pool.query('SELECT * FROM files WHERE id = ?', [req.params.id], (err, results) => {
        if (err) return res.status(500).send(err.message);
...
const start = async () => {
    // this is only for auto import database schema (on each run)
    {
        ...
        const fileData = fs.readFileSync(path.join(__dirname, 'schema.sql')).toString();
